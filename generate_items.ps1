# generate_items.ps1 - Lê items_data.tsv e gera .tres + C# seed
# Uso: powershell -ExecutionPolicy Bypass -File generate_items.ps1
# Opcoes: -GenerateTres, -GenerateSeed, -ShowSummary
...
# Remove acentos de uma string (para lookup no dicionario)
function Normalize-Tipo {
    param($tipo)
    $map = @{}
    "aA" | ForEach-Object { $c = $_[0]; $C = $_[1]; for ($x = 0xE0; $x -le 0xE5; $x++) { $map[[char]$x] = $c }; for ($x = 0xC0; $x -le 0xC5; $x++) { $map[[char]$x] = $C } }
    $map[[char]0xE7] = 'c'; $map[[char]0xC7] = 'C'
    "eE" | ForEach-Object { $c = $_[0]; $C = $_[1]; foreach ($x in @(0xE8,0xE9,0xEA,0xEB)) { $map[[char]$x] = $c }; foreach ($x in @(0xC8,0xC9,0xCA,0xCB)) { $map[[char]$x] = $C } }
    "iI" | ForEach-Object { $c = $_[0]; $C = $_[1]; foreach ($x in @(0xEC,0xED,0xEE,0xEF)) { $map[[char]$x] = $c }; foreach ($x in @(0xCC,0xCD,0xCE,0xCF)) { $map[[char]$x] = $C } }
    "oO" | ForEach-Object { $c = $_[0]; $C = $_[1]; for ($x = 0xF2; $x -le 0xF6; $x++) { $map[[char]$x] = $c }; for ($x = 0xD2; $x -le 0xD6; $x++) { $map[[char]$x] = $C } }
    "uU" | ForEach-Object { $c = $_[0]; $C = $_[1]; foreach ($x in @(0xF9,0xFA,0xFB,0xFC)) { $map[[char]$x] = $c }; foreach ($x in @(0xD9,0xDA,0xDB,0xDC)) { $map[[char]$x] = $C } }
...
$rawConfig = @{
    "Arco" = @{
        ItemType = 7; DuasMaos = $true
        Fixo1 = "DanoFisico"; Fixo2 = "Destreza"; Fixo3 = $null
        F2RatioMin = 0.40; F2RatioMax = 0.60
        EhEscudo = $false
    ...
    "Adaga Secundaria" = @{
        ItemType = 7; DuasMaos = $false
        Fixo1 = "Destreza"; Fixo2 = "Agilidade"; Fixo3 = $null
        F2RatioMin = 0.70; F2RatioMax = 0.90
        EhEscudo = $false
        Pool = "ChanceCritica,VelocidadeAtaque,Precisao,Agilidade,Evasao,RouboVida,VelocidadeMovimento,Tenacidade"
    }
    "Machado" = @{
        ItemType = 7; DuasMaos = $true
        Fixo1 = "DanoFisico"; Fixo2 = "Forca"; Fixo3 = $null
        F2RatioMin = 0.40; F2RatioMax = 0.60
        EhEscudo = $false
    ...
    "Escudo Guardiao" = @{
        ItemType = 8; DuasMaos = $false
        Fixo1 = "DefesaFisica"; Fixo2 = "Forca"; Fixo3 = "Agilidade"
        F2RatioMin = 0.50; F2RatioMax = 0.70
        F3RatioMin = 0.40; F3RatioMax = 0.60
        EhEscudo = $true
        Pool = "Tenacidade,DefesaFisica,DefesaMagica,Hp,RegeneracaoVida,Evasao,Precisao"
    }
    "Cajado" = @{
        ItemType = 7; DuasMaos = $true
        Fixo1 = "DanoFisico"; Fixo2 = "Inteligencia"; Fixo3 = $null
        F2RatioMin = 0.40; F2RatioMax = 0.60
        EhEscudo = $false
    ...
    "Martelo" = @{
        ItemType = 7; DuasMaos = $true
        Fixo1 = "DanoFisico"; Fixo2 = "Forca"; Fixo3 = "Inteligencia"
        F2RatioMin = 0.30; F2RatioMax = 0.50
        F3RatioMin = 0.30; F3RatioMax = 0.50
        EhEscudo = $false
        Pool = "ChanceCritica,DanoCriticoBonus,Precisao,Tenacidade,RouboVida,RouboMana,RegeneracaoVida,RegeneracaoMana"
    }
    "Escudo Clerigo" = @{
        ItemType = 8; DuasMaos = $false
        Fixo1 = "DefesaFisica"; Fixo2 = "Forca"; Fixo3 = "Inteligencia"
        F2RatioMin = 0.50; F2RatioMax = 0.70
        F3RatioMin = 0.40; F3RatioMax = 0.60
        EhEscudo = $true
...
function Read-ItemsTsv {
    ...
    if (-not (Test-Path $tsvPath)) {
        Write-Error "Arquivo items_data.tsv nao encontrado em $tsvPath"
        return $null
    }

    $lines = Get-Content -Path $tsvPath -Encoding UTF8
    if ($lines.Count -lt 2) {
        Write-Error "TSV vazio ou apenas cabecalho"
        return $null
    }
...
# Calcula ranges de stats secundarios
...
# Adiciona linha de stat no .tres
...
function Write-CSharpSeed {
    ...
    [void]$sb.AppendLine("// Cole dentro de DatabaseManager ou use como partial class.")
    [void]$sb.AppendLine("// ============================================================")
    [void]$sb.AppendLine("partial class DatabaseManager")
    [void]$sb.AppendLine("{")
    [void]$sb.AppendLine("    public void SeedItemDefinitions()")
    [void]$sb.AppendLine("    {")
    [void]$sb.AppendLine("        // Consumiveis principais")
    [void]$sb.AppendLine("        var coreConsumables = new List<ItemDefinition>")
    [void]$sb.AppendLine("        {")

    # Use single quotes to avoid $"" issues in PS5
    [void]$sb.AppendLine('            new() { Id = 100, Name = "Pergaminho do Pet", Type = ItemType.Consumable, MaxStack = 99, IsStackable = true, BuyPrice = 50 },')
    [void]$sb.AppendLine('            new() { Id = 101, Name = "Pergaminho de Ressurreicao", Type = ItemType.Consumable, MaxStack = 99, IsStackable = true, BuyPrice = 100 },')
    [void]$sb.AppendLine('            new() { Id = ItemDefinitions.PergaminhoCriacaoCla, Name = "Pergaminho de Criacao de Cla", Type = ItemType.Consumable, MaxStack = 99, IsStackable = true, BuyPrice = 100 },')
    ...
    [void]$sb.AppendLine('            Logger.Info($"Item definitions ja existem ({count} registros). Consumiveis atualizados.");')
    ...
    [void]$sb.AppendLine("            SaveItemDefinition(item);")
    [void]$sb.AppendLine('        Logger.Info($"{items.Count} definicoes inseridas no banco.");')
    [void]$sb.AppendLine("    }")
    [void]$sb.AppendLine("}")
