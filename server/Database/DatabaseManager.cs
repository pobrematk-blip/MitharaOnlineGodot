...
public class DatabaseManager
{
    ...
    public void Initialize()
    {
        ...
        TryAddColumn(conn, "accounts", "security_question", "TEXT NOT NULL DEFAULT ''");
        TryAddColumn(conn, "accounts", "security_answer", "TEXT NOT NULL DEFAULT ''");
        TryAddColumn(conn, "accounts", "salt", "TEXT NOT NULL DEFAULT ''");
        TryAddColumn(conn, "characters", "pos_x", "REAL NOT NULL DEFAULT 1000");
    ...
    public int? CreateAccount(string username, string password, string securityQuestion = "", string securityAnswer = "")
    {
        ...
        cmd.CommandText = "INSERT INTO accounts (username, password_hash, security_question, security_answer, salt) VALUES (@u, @p, @q, @a, @s)";
        ...
        getId.CommandText = "SELECT id FROM accounts WHERE username = @u";
        getId.Parameters.AddWithValue("@u", username);
        return (int)(long)getId.ExecuteScalar()!;
    }
    ...
    public int CreateCharacter(int accountId, string name, string className, string race)
    {
        ...
        using var getId = conn.CreateCommand();
        getId.CommandText = "SELECT id FROM characters WHERE account_id = @a AND slot_index = @s";
        getId.Parameters.AddWithValue("@a", accountId);
        getId.Parameters.AddWithValue("@s", slotIndex);
    ...
    public List<Mithara.Server.Entities.ItemInstance> LoadItems(int characterId)
    {
        ...
        conn.Open();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT id, item_id, slot, quantity FROM items WHERE character_id = @c ORDER BY slot";
    ...
    public void SaveItem(int characterId, Mithara.Server.Entities.ItemInstance item)
    {
        ...
        {
            using var cmd = conn.CreateCommand();
            cmd.CommandText = "UPDATE items SET slot = @s, quantity = @q WHERE id = @i AND character_id = @c";
        ...
        {
            using var cmd = conn.CreateCommand();
            cmd.CommandText = "INSERT INTO items (character_id, slot, item_id, quantity) VALUES (@c, @s, @ii, @q)";
            cmd.Parameters.AddWithValue("@c", characterId);
            cmd.Parameters.AddWithValue("@s", item.Slot);
            ...
            item.DbId = Convert.ToInt32(getId.ExecuteScalar());
        }
    }
    ...
    public void SavePlayerQuest(int characterId, int questId, string progress, bool completed, bool claimed)
    {
        ...
        cmd.CommandText = """
            ...
            VALUES (@c, @q, @p, @co, @cl)
            ON CONFLICT(id) DO UPDATE SET progress = @p, completed = @co, claimed = @cl
            """;
        cmd.Parameters.AddWithValue("@c", characterId);
    ...
    public void UpsertPlayerQuest(int characterId, int questId, string progress, bool completed, bool claimed)
    {
        ...
        if (existingId != null)
        {
            ...
            cmd.Parameters.AddWithValue("@i", (int)(long)existingId);
        }
        else
        {
